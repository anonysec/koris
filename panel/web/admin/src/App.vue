...
<template>
  <!-- Loading -->
  <div v-if="screen === 'loading'" class="loading-screen"><div class="spinner"></div><p>Loading...</p></div>

  <!-- Auth -->
  <div v-else-if="screen === 'setup' || screen === 'login'" class="auth-screen">
    <div class="auth-hero">
      <div class="brand"><div class="logo">K</div><div><h1 style="font-size:17px">KorisPanel</h1><span>Control Center</span></div></div>
    ...
    <div class="auth-card">
      ...
      <div class="sub">{{ screen === 'setup' ? 'Set up initial owner account' : 'Admin credentials required' }}</div>
  ...
  <template v-else>
    <aside class="sidebar">
      <div class="brand"><div class="logo">K</div><div><h1>KorisPanel</h1><span>v{{ health?.version || 'dev' }}</span></div></div>
      ...
      <button class="nav-item" :class="{active:section==='payments'}" @click="section='payments'"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 7h18M3 12h18M3 17h12"/></svg>Transactions<span v-if="stats.pending_payments" class="badge">{{ stats.pending_payments }}</span></button>
      ...
      <button class="nav-item" :class="{active:section==='tickets'}" @click="section='tickets'"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>Tickets<span v-if="stats.open_tickets" class="badge">{{ stats.open_tickets }}</span></button>
      ...
      <div class="sidebar-foot">
        ...
        <button class="icon-btn" style="width:30px;height:30px;margin-left:auto;border-radius:8px" @click="logout"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="width:14px;height:14px"><path d="M9 21H5a2 2 0 01-2-2V5a2 2 0 012-2h4M16 17l5-5-5-5M21 12H9"/></svg></button>
      </div>
    </aside>
    ...
    <main class="main">
      ...
      <div class="topbar">
        <div>
          <h2>{{ section==='overview'?'Dashboard':section==='customers'?'Users':section==='customer-detail'?'User Detail':section==='payments'?'Transactions':section==='plans'?'Billing':section==='nodes'?'Services':section==='tickets'?'Tickets':section==='resellers'?'Users':section==='system'?'Settings':'Panel' }}</h2>
          <p>{{ section==='overview'?`Welcome back, ${user.username}`:section==='customers'?'Manage accounts and resellers':section==='payments'?'Payment history and approvals':section==='plans'?'Subscription plans and pricing':section==='nodes'?'Server nodes and VPN configuration':section==='tickets'?'Support tickets':section==='system'?'Panel configuration and system tools':'Details' }}</p>
        </div>
        <div class="search"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="7"/><path d="M21 21l-4-4"/></svg><input v-model="search" @keyup.enter="loadDashboard" placeholder="Search..."></div>
        ...
        <button class="btn-primary" @click="customerModalOpen=true">+ New User</button>
      </div>
      <p v-if="error" class="alert danger">{{ error }}</p>
      ...
      <div v-if="section==='overview'" class="page-content">
        ...
        <div class="grid charts">
          <div class="card"><div class="card-head"><div><h4>Usage Monitor</h4><div class="sub">Admin traffic usage over time</div></div><span class="pill" :class="realtimeConnected?'ok':'bad'">{{ realtimeConnected?'Live':'Offline' }}</span></div><div class="area-wrap"><svg viewBox="0 0 360 60" preserveAspectRatio="none" style="width:100%;height:100%"><polyline fill="none" stroke="var(--brand)" stroke-width="2" :points="rxPoints"/><polyline fill="none" stroke="var(--brand-2)" stroke-width="2" :points="txPoints"/></svg></div><div class="legend"><span><i style="background:var(--brand)"></i>Download {{ formatBytes((stats.total_rx_bps||0)/8) }}/s</span><span><i style="background:var(--brand-2)"></i>Upload {{ formatBytes((stats.total_tx_bps||0)/8) }}/s</span></div></div>
        ...
        <!-- Recent Users table -->
        <div class="card table-wrap"><div class="card-head"><div><h4>Recent Users</h4><div class="sub">Latest accounts</div></div><button class="btn-ghost" @click="section='customers'">View All</button></div><table><thead><tr><th>User</th><th>Role</th><th>Status</th><th>Storage Used</th><th>Last Active</th></tr></thead><tbody><tr v-for="c in customers.slice(0,5)" :key="c.id" style="cursor:pointer" @click="openCustomer(c)"><td><div class="uname"><div class="u" :style="{background:`linear-gradient(135deg,hsl(${c.id*37%360},70%,55%),hsl(${c.id*67%360},70%,45%))`}">{{ c.username.slice(0,1).toUpperCase() }}</div><div>{{ c.username }}<br><span style="color:var(--muted);font-size:12px">{{ c.display_name||'—' }}</span></div></div></td><td>{{ c.plan||'Free' }}</td><td><span class="pill" :class="c.status==='active'?'ok':c.status==='disabled'?'bad':'warn'">{{ c.status }}</span></td><td><div class="prog"><i :style="{width:(c.credit>0?Math.min(c.credit/100,100):20)+'%',background:c.credit>50?'var(--amber)':'var(--green)'}"></i></div><span style="color:var(--muted);font-size:12px">{{ formatMoney(c.credit) }}</span></td><td style="color:var(--muted)">{{ formatDate(c.created_at) }}</td></tr></tbody></table></div>
      </div>

      <!-- USERS (with resellers sub-tab) -->
      <div v-else-if="section==='customers'||section==='resellers'" class="page-content">
        <div class="tabs" style="margin-bottom:18px">
          <button :class="{on:section==='customers'&&customerView==='active'}" @click="section='customers';customerView='active'">Active Users</button>
        ...
        <template v-if="section==='customers'">
          <div class="card table-wrap"><div class="card-head"><h4>{{ filteredCustomers.length }} users</h4><button class="btn-primary" style="padding:8px 14px" @click="customerModalOpen=true">+ New User</button></div><table><thead><tr><th>User</th><th>Status</th><th>Plan</th><th>Wallet</th><th>Created</th><th></th></tr></thead><tbody><tr v-for="c in filteredCustomers" :key="c.id"><td><div class="uname"><div class="u" :style="{background:`linear-gradient(135deg,hsl(${c.id*37%360},70%,55%),hsl(${c.id*67%360},70%,45%))`}">{{ c.username.slice(0,2).toUpperCase() }}</div><div>{{ c.username }}<br><span style="color:var(--muted);font-size:12px">{{ c.display_name||'—' }}</span></div></div></td><td><span class="pill" :class="c.status==='active'?'ok':c.status==='disabled'?'bad':'warn'">{{ c.status }}</span></td><td>{{ c.plan||'—' }}</td><td>{{ formatMoney(c.credit) }}</td><td style="color:var(--muted)">{{ formatDate(c.created_at) }}</td><td><button v-if="customerView==='active'" class="btn-ghost" style="padding:5px 10px;font-size:12px" @click="openCustomer(c)">Detail</button><button v-else class="btn-primary" style="padding:5px 10px;font-size:12px" @click="restoreDeletedCustomer(c as any)">Restore</button></td></tr><tr v-if="!filteredCustomers.length"><td colspan="6" style="text-align:center;color:var(--muted);padding:28px">No users found</td></tr></tbody></table></div>
        </template>

        <!-- Resellers -->
        <template v-else>
          <div class="grid" style="grid-template-columns:360px 1fr"><div class="card"><div class="card-head"><h4>New Reseller</h4></div><form class="form-stack" @submit.prevent="createReseller"><label>Username<input v-model.trim="resellerForm.username" required/></label><label>Password<input v-model="resellerForm.password" type="password" required/></label><button class="btn-primary" :disabled="busy">Create</button></form></div><div class="card table-wrap"><div class="card-head"><h4>Reseller Fleet</h4></div><table><thead><tr><th>Username</th><th>Credit</th><th>Status</th><th>Adjust</th><th></th></tr></thead><tbody><tr v-for="r in resellersList" :key="r.id"><td><b>{{ r.username }}</b></td><td>{{ formatMoney(r.credit) }}</td><td><span class="pill" :class="r.is_active?'ok':'bad'">{{ r.is_active?'Active':'Inactive' }}</span></td><td style="display:flex;gap:4px;align-items:center"><input v-model.number="resellerCreditForm[r.id]" type="number" style="width:80px;min-height:30px"/><button class="btn-ghost" style="padding:4px 8px;font-size:11px" @click="adjustResellerCredit(r.id,true)">+</button><button class="btn-danger" style="padding:4px 8px;font-size:11px" @click="adjustResellerCredit(r.id,false)">-</button></td><td><button class="btn-danger" style="padding:4px 8px;font-size:11px" @click="deleteReseller(r.id)">Del</button></td></tr></tbody></table></div></div>
        </template>
      </div>

      <!-- USER DETAIL -->
      <div v-else-if="section==='customer-detail'" class="page-content">
        <button class="btn-ghost" style="margin-bottom:14px" @click="section='customers';selectedCustomer=null">← Back</button>
        <div v-if="detailLoading" style="text-align:center;color:var(--muted);padding:40px">Loading...</div>
        <div v-else-if="selectedCustomer">
          <div class="detail-hero"><div class="avatar" :style="{background:`linear-gradient(135deg,hsl(${selectedCustomer.id*37%360},70%,55%),hsl(${selectedCustomer.id*67%360},70%,45%))`,width:'56px',height:'56px',borderRadius:'14px',fontSize:'18px'}">{{ selectedCustomer.username.slice(0,2).toUpperCase() }}</div><div style="flex:1"><h3 style="font-size:20px;font-weight:700">{{ selectedCustomer.username }}</h3><p style="color:var(--muted);font-size:13px;margin-top:2px">{{ selectedCustomer.display_name||'' }} · {{ selectedCustomer.plan||'No plan' }} · <span class="pill" :class="selectedCustomer.status==='active'?'ok':'warn'">{{ selectedCustomer.status }}</span></p></div><div style="text-align:right"><div style="color:var(--muted);font-size:12px">Wallet</div><div style="font-size:22px;font-weight:700">{{ formatMoney(selectedCustomer.credit) }}</div></div></div>
      ...
      <div v-else-if="section==='system'" class="page-content">
        <div class="settings-grid">
          ...
          <div v-if="systemTab==='diagnostics'" class="card"><div class="card-head"><div><h4>Panel Status</h4><div class="sub">System health and diagnostics</div></div><button class="btn-ghost" :disabled="diagnosticsLoading" @click="loadDiagnostics">{{ diagnosticsLoading?'Checking...':'Refresh' }}</button></div><div v-if="diagnosticsData"><div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:16px"><div style="padding:12px;border:1px solid var(--border);border-radius:10px"><div style="color:var(--muted);font-size:11px;text-transform:uppercase">Disk</div><b>{{ diagnosticsData.disk }}</b></div><div style="padding:12px;border:1px solid var(--border);border-radius:10px"><div style="color:var(--muted);font-size:11px;text-transform:uppercase">Memory</div><b>{{ diagnosticsData.mem }}</b></div></div><table><thead><tr><th>Service</th><th>Status</th></tr></thead><tbody><tr v-for="c in diagnosticsData.checks" :key="c.name"><td><b>{{ c.name }}</b></td><td><span class="pill" :class="c.ok?'ok':'bad'">{{ c.ok?'OK':'Issue' }}</span></td></tr></tbody></table></div><p v-else style="color:var(--muted);text-align:center;padding:20px">Click Refresh to run diagnostics</p></div>
          <div v-else-if="systemTab==='audit'" class="card table-wrap"><div class="card-head"><h4>Audit Logs</h4><div style="display:flex;gap:6px"><button class="btn-ghost" style="padding:5px 10px;font-size:11px" @click="auditOffset=Math.max(0,auditOffset-auditLimit);loadAuditLogs()">← Prev</button><button class="btn-ghost" style="padding:5px 10px;font-size:11px" @click="auditOffset+=auditLimit;loadAuditLogs()">Next →</button></div></div><table><thead><tr><th>Actor</th><th>Action</th><th>Entity</th><th>IP</th><th>Date</th></tr></thead><tbody><tr v-for="log in auditLogs" :key="log.id"><td>{{ log.actor }}</td><td><span class="pill warn">{{ log.action }}</span></td><td>{{ log.entity_type }} #{{ log.entity_id }}</td><td style="color:var(--muted)">{{ log.ip }}</td><td style="color:var(--muted)">{{ formatDate(log.created_at) }}</td></tr></tbody></table></div>
          <div v-else class="card"><div class="card-head"><h4>Backup & Export</h4></div><p style="color:var(--muted);margin-bottom:14px;font-size:13px">Download CSV snapshots. Automated database backups run daily at 2:00 AM.</p><div class="action-row"><button class="btn-primary" @click="exportCSV('customers')">Users CSV</button><button class="btn-primary" @click="exportCSV('payments')">Payments CSV</button><button class="btn-primary" @click="exportCSV('radacct')">Sessions CSV</button><button class="btn-primary" @click="exportCSV('wallet-transactions')">Wallet CSV</button></div></div>
        </div>
      </div>
    </main>

    <!-- Modals -->
    <div v-if="customerModalOpen" class="modal-backdrop" @click.self="customerModalOpen=false"><div class="modal-card"><div class="card-head"><h4>New User</h4><button class="icon-btn" style="width:32px;height:32px;border-radius:8px" @click="customerModalOpen=false">✕</button></div><form class="form-stack" @submit.prevent="createCustomer"><div class="two-col"><label>Username<input v-model.trim="createForm.username" required/></label><label>Password<input v-model="createForm.password" required/></label></div><label>Display Name<input v-model.trim="createForm.display_name"/></label><label>Plan<select v-model.number="createForm.plan_id" @change="applyCreatePlan"><option :value="0">No plan</option><option v-for="p in activePlans" :key="p.id" :value="p.id">{{ p.name }}</option></select></label><div class="two-col"><label>Data GB (0=unlimited)<input v-model.number="createForm.data_gb" type="number" min="0"/></label><label>Speed Mbps (0=unlimited)<input v-model.number="createForm.speed_mbps" type="number" min="0"/></label></div><label>Duration Days<input v-model.number="createForm.days" type="number" min="0"/></label><div class="action-row"><button class="btn-primary" :disabled="busy">{{ busy?'Creating...':'Create' }}</button><button type="button" class="btn-ghost" @click="customerModalOpen=false">Cancel</button></div></form></div></div>
    <div v-if="nodeModalOpen" class="modal-backdrop" @click.self="nodeModalOpen=false"><div class="modal-card"><div class="card-head"><h4>New Node</h4><button class="icon-btn" style="width:32px;height:32px;border-radius:8px" @click="nodeModalOpen=false">✕</button></div><form class="form-stack" @submit.prevent="createNode"><div class="two-col"><label>Name<input v-model.trim="nodeForm.name" required/></label><label>Public IP<input v-model.trim="nodeForm.public_ip" required/></label></div><label>Domain<input v-model.trim="nodeForm.domain"/></label><button class="btn-primary" :disabled="busy">Create</button></form><div v-if="nodeToken" style="margin-top:16px;border:1px solid rgba(91,157,255,.3);border-radius:10px;padding:14px;background:rgba(91,157,255,.04)"><small style="color:var(--brand);font-weight:700">Token (copy now):</small><code style="display:block;margin-top:6px;word-break:break-all;background:var(--surface-2);padding:8px;border-radius:6px">{{ nodeToken }}</code><button class="btn-ghost" style="margin-top:8px;padding:6px 12px;font-size:12px" @click="copyToClipboard(nodeToken)">Copy</button></div></div></div>
    <div v-if="planModalOpen" class="modal-backdrop" @click.self="planModalOpen=false"><div class="modal-card"><div class="card-head"><h4>{{ editingPlanId?'Edit Plan':'New Plan' }}</h4><button class="icon-btn" style="width:32px;height:32px;border-radius:8px" @click="planModalOpen=false">✕</button></div><form class="form-stack" @submit.prevent="savePlan"><label>Name<input v-model.trim="planForm.name" required/></label><div class="two-col"><label>Data GB<input v-model.number="planForm.data_gb" type="number" min="0"/></label><label>Speed Mbps<input v-model.number="planForm.speed_mbps" type="number" min="0"/></label></div><div class="two-col"><label>Days<input v-model.number="planForm.duration_days" type="number" min="0"/></label><label>Price<input v-model.number="planForm.price" type="number" min="0"/></label></div><label style="display:flex;align-items:center;gap:8px;flex-direction:row"><input v-model="planForm.is_active" type="checkbox" style="width:18px;min-height:18px"/>Active</label><div class="action-row"><button class="btn-primary" :disabled="busy">{{ editingPlanId?'Update':'Create' }}</button><button type="button" class="btn-ghost" @click="planModalOpen=false">Cancel</button></div></form></div></div>
  </template>
</template>
