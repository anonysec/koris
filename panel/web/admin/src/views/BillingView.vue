  ...
  <div class="page billing-view">
    ...
    <section class="panel invoices-section">
        ...
        <table class="invoice-table" role="table">
          ...
          <tbody>
            <tr v-for="inv in invoices" :key="inv.id">
              ...
              <td>
                <KStatusPill :status="invoiceStatusVariant(inv.status)" size="sm">
                  {{ inv.status }}
                </KStatusPill>
              </td>
              ...
              <td>
                <KButton variant="ghost" size="sm" @click="downloadInvoicePdf(inv)">
                  📥 PDF
                </KButton>
              </td>
            </tr>
      ...
      <div v-if="invoiceTotal > invoicePageSize" class="pagination">
        <KButton variant="ghost" size="sm" :disabled="invoicePage <= 1" @click="prevInvoicePage">
          ← {{ t('billing.prev') }}
        </KButton>
        <span class="pagination__info">{{ invoicePage }} / {{ invoiceTotalPages }}</span>
        <KButton variant="ghost" size="sm" :disabled="invoicePage >= invoiceTotalPages" @click="nextInvoicePage">
          {{ t('billing.next') }} →
        </KButton>
      </div>
    </section>
    ...
    <section class="panel gateways-section">
      <div class="panel-header">
        ...
        <KButton variant="primary" size="sm" @click="showGatewayDrawer = true">
          {{ t('billing.add_gateway') }}
        </KButton>
      </div>

      <div v-if="gatewaysLoading" class="gateway-skeleton">
        <KSkeleton variant="card" :count="2" />
      </div>
      ...
      <div v-else class="gateway-grid">
        <div v-for="gw in gateways" :key="gw.id" class="gateway-card">
          <div class="gateway-card__header">
            ...
            <KStatusPill :status="gw.is_active ? 'active' : 'disabled'" size="sm" />
          </div>
    ...
    <!-- ═══ Add Gateway Drawer ═══ -->
    <KDrawer :open="showGatewayDrawer" :title="t('billing.add_gateway')" side="right" @close="showGatewayDrawer = false">
      <form class="gateway-form" @submit.prevent="submitGateway">
        <KFormField name="gw-name" :label="t('billing.gw_name')" required>
          <template #default="{ fieldId }">
            <KInput :id="fieldId" v-model="gatewayForm.name" placeholder="My Gateway" />
          </template>
        </KFormField>
        <KFormField name="gw-type" :label="t('billing.gw_type')" required>
          <template #default="{ fieldId }">
            <KSelect :id="fieldId" v-model="gatewayForm.type" :options="gatewayTypeOptions" />
          </template>
        </KFormField>

        <!-- Zarinpal config -->
        <template v-if="gatewayForm.type === 'zarinpal'">
          <KFormField name="gw-merchant" :label="t('billing.gw_merchant_id')" required>
            <template #default="{ fieldId }">
              <KInput :id="fieldId" v-model="gatewayForm.merchant_id" placeholder="xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx" />
            </template>
          </KFormField>
        </template>
        ...
        <template v-if="gatewayForm.type === 'crypto'">
          <KFormField name="gw-wallet" :label="t('billing.gw_wallet_address')" required>
            <template #default="{ fieldId }">
              <KInput :id="fieldId" v-model="gatewayForm.wallet_address" placeholder="0x..." />
            </template>
          </KFormField>
          <KFormField name="gw-network" :label="t('billing.gw_network')">
            <template #default="{ fieldId }">
              <KInput :id="fieldId" v-model="gatewayForm.network" placeholder="TRC20" />
            </template>
          </KFormField>
        </template>
        ...
        <template v-if="gatewayForm.type === 'manual'">
          <KFormField name="gw-instructions" :label="t('billing.gw_instructions')">
            <template #default="{ fieldId }">
              <KInput :id="fieldId" v-model="gatewayForm.instructions" placeholder="Payment instructions..." />
            </template>
          </KFormField>
        </template>

        <KButton type="submit" variant="primary" :loading="savingGateway" full-width>
