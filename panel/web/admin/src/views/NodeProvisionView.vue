  ...
  <div class="page provision-view">
    ...
    <section v-if="!provisionStarted" class="provision-form-section">
      <form class="provision-form" @submit.prevent="startProvision">
        <div class="form-grid">
          <KFormField name="prov-host" :label="t('provision.host')" required>
            <template #default="{ fieldId }">
              <KInput :id="fieldId" v-model="form.host" placeholder="192.168.1.100" />
            </template>
          </KFormField>
          <KFormField name="prov-port" :label="t('provision.port')" required>
            <template #default="{ fieldId }">
              <KInput :id="fieldId" v-model.number="form.port" type="number" placeholder="22" />
            </template>
          </KFormField>
          <KFormField name="prov-user" :label="t('provision.user')" required>
            <template #default="{ fieldId }">
              <KInput :id="fieldId" v-model="form.user" placeholder="root" />
            </template>
          </KFormField>
          <KFormField name="prov-auth" :label="t('provision.auth_method')">
            <template #default="{ fieldId }">
              <KSelect
                :id="fieldId"
        ...
        <KFormField v-if="form.auth_method === 'password'" name="prov-password" :label="t('provision.password')" required>
          <template #default="{ fieldId }">
            <KInput :id="fieldId" v-model="form.password" type="password" :placeholder="t('provision.password_placeholder')" />
          </template>
        </KFormField>

        <KFormField v-if="form.auth_method === 'key'" name="prov-key" :label="t('provision.ssh_key')" required>
          <template #default="{ fieldId }">
            <textarea
              :id="fieldId"
        ...
        <KFormField name="prov-group" :label="t('provision.target_group')">
          <template #default="{ fieldId }">
            <KSelect :id="fieldId" v-model="form.group_id" :options="groupOptions" :placeholder="t('provision.no_group')" />
          </template>
        </KFormField>

        <div class="form-actions">
          <KButton type="submit" variant="primary" :disabled="!canSubmit" :loading="provisioning">
            {{ t('provision.start') }}
          </KButton>
        </div>
      </form>
