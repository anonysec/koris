...
<template>
  <div class="page protocols-view">
    <header class="page-header">
      ...
      <KSelect v-if="nodeOptions.length" :options="nodeOptions" v-model="selectedNodeId" placeholder="Select node" class="node-select" />
    </header>

    <div v-if="loadingNodes" class="loading-grid">
      <KSkeleton v-for="i in 5" :key="i" height="104px" />
    </div>

    <KEmptyState v-else-if="!nodes.length" title="No nodes yet" description="Add a node to start configuring protocols." />
        ...
        <div v-for="proto in protocols" :key="proto.key" class="protocol-card" :class="{ enabled: config[proto.key]?.enabled }">
          <div class="card-top">
            ...
            <div class="proto-meta">
              ...
              <KStatusPill :status="config[proto.key]?.enabled ? 'active' : 'inactive'" :label="config[proto.key]?.enabled ? 'Enabled' : 'Disabled'" />
            </div>
          ...
          <KButton variant="ghost" size="sm" class="edit-btn" @click="openProtocolPanel(proto.key)">Configure</KButton>
        </div>
      </div>
    </template>
    ...
    <KSlideOver :open="panelOpen" :title="panelTitle" width="440px" @close="closePanel">
        ...
        <template v-for="field in currentFields" :key="field.key">
          <div v-if="!field.showIf || field.showIf(panelForm)" class="panel-field">
            <KFormField :label="t(field.label)" :name="field.key">
              <KSelect v-if="field.type === 'select'" v-model="panelForm[field.key]" :options="field.options || []" />
              ...
              <div v-else-if="field.type === 'dns'" class="dns-field">
                <div class="dns-input-wrap">
                  <KInput v-model="panelForm[field.key]" type="text" placeholder="8.8.8.8" />
              ...
              <div v-else-if="field.type === 'password' || field.key === 'psk'" class="password-field">
                <KInput v-model="panelForm[field.key]" type="text" autocomplete="off" :placeholder="t(field.label)" />
                <KButton size="sm" variant="ghost" @click="generatePsk">{{ t('services.auto_generate') }}</KButton>
              </div>
              <KInput v-else v-model="panelForm[field.key]" :type="field.type" :placeholder="String(field.default || '')" />
            </KFormField>
          </div>
        </template>
      </div>
      <template #footer>
        <div class="panel-footer">
          ...
          <KButton :loading="panelSaving" @click="saveProtocolSettings">{{ t('nodes.save_config') }}</KButton>
        </div>
      </template>
    </KSlideOver>
  </div>
