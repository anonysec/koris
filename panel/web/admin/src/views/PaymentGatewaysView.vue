  ...
  <div class="page gateways-view">
    <section class="panel">
      <div class="panel-header">
        ...
        <KButton variant="primary" size="sm" @click="openAdd">
      ...
      <div v-if="loading" class="skeleton-wrap">
        <KSkeleton variant="table-row" :count="3" />
      </div>
            ...
            <tr v-for="gw in gateways" :key="gw.id">
              ...
              <td>
                <div class="action-btns">
                  <KButton variant="ghost" size="sm" @click="openEdit(gw)">
                    {{ t('gateways.edit') }}
                  </KButton>
                  <KButton variant="ghost" size="sm" @click="toggleActive(gw)">
                  ...
                  <template v-else>
                    <KButton variant="danger" size="sm" @click="deleteGateway(gw.id)">
                      {{ t('gateways.confirm') }}
                    </KButton>
                    <KButton variant="ghost" size="sm" @click="confirmDeleteId = null">
                      {{ t('btn.cancel') }}
                    </KButton>
                  </template>
                </div>
    ...
    <!-- Add/Edit Drawer -->
    <KDrawer :open="showDrawer" :title="drawerTitle" side="right" @close="closeDrawer">
      <form class="gateway-form" @submit.prevent="submitGateway">
        <KFormField name="gw-name" :label="t('gateways.form_name')" required>
          <template #default="{ fieldId }">
            <KInput :id="fieldId" v-model="form.name" placeholder="zarinpal" :disabled="!!editingId" />
          </template>
        </KFormField>

        <KFormField name="gw-display-name" :label="t('gateways.form_display_name')" required>
          <template #default="{ fieldId }">
            <KInput :id="fieldId" v-model="form.display_name" placeholder="Zarinpal" />
          </template>
        </KFormField>

        <KFormField name="gw-config" :label="t('gateways.form_config_json')">
          <template #default="{ fieldId }">
            <textarea
              :id="fieldId"
        ...
        <KButton type="submit" variant="primary" :loading="saving" full-width>
