...
func (s *Server) listCustomerNotes(w http.ResponseWriter, _ *http.Request, customerID int64) {
	...
	rows, err := s.DB.Query(`SELECT id, admin_username, body, created_at FROM user_notes WHERE customer_id=? ORDER BY created_at DESC`, customerID)
	if err != nil {
...
func (s *Server) createCustomerNote(w http.ResponseWriter, r *http.Request, customerID int64) {
	...
	res, err := s.DB.Exec(`INSERT INTO user_notes(customer_id, admin_username, body) VALUES(?,?,?)`,
