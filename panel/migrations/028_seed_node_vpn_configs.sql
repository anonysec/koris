...
SELECT n.id, 'openvpn', 1, 1194, '10.8.0.0/24', '{"transport":"udp","cipher":"AES-256-GCM","tls_mode":"tls-crypt","dns1":"8.8.8.8","dns2":"8.8.4.4","comp_lzo":false,"topology":"subnet","verb":3,"keepalive":"10 120"}'
FROM nodes n LEFT JOIN node_vpn_configs c ON c.node_id = n.id AND c.protocol = 'openvpn'
WHERE c.id IS NULL;

INSERT IGNORE INTO node_vpn_configs (node_id, protocol, enabled, port, network, extra_json)
SELECT n.id, 'l2tp', 0, 1701, '10.9.0.0/24', '{"ipsec_mode":"ipsec","psk":"","auth_method":"CHAP","dns1":"8.8.8.8","dns2":"8.8.4.4","lcp_echo_interval":30,"lcp_echo_failure":4}'
FROM nodes n LEFT JOIN node_vpn_configs c ON c.node_id = n.id AND c.protocol = 'l2tp'
WHERE c.id IS NULL;

INSERT IGNORE INTO node_vpn_configs (node_id, protocol, enabled, port, network, extra_json)
SELECT n.id, 'ikev2', 0, 500, '10.10.0.0/24', '{"auth_type":"psk","psk":"","dns1":"8.8.8.8","dns2":"8.8.4.4","dpd_interval":30,"dpd_timeout":150,"rekey_time":"4h","ike_proposals":"aes256-sha256-modp2048","esp_proposals":"aes256-sha256"}'
...
INSERT IGNORE INTO node_vpn_configs (node_id, protocol, enabled, port, network, extra_json)
SELECT n.id, 'wireguard', 0, 51820, '10.66.66.0/24', '{"dns_1":"1.1.1.1","dns_2":"8.8.8.8","gaming_optimize":false}'
