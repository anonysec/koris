...
PANEL_ADDR="${PANEL_ADDR:-$(grep -E '^PANEL_ADDR=' /etc/koris/panel.env 2>/dev/null | cut -d= -f2 | tr -d "'"\" || true)}"
...
# Collect diagnostics
PANEL_LOGS=$(journalctl -u koris -n 30 --no-pager -o short-iso 2>&1 || echo "not available")
...
NODE_AGENT_LOGS=$(journalctl -u knode -n 15 --no-pager -o short-iso 2>&1 || echo "not available")
SERVICE_STATUS=$(systemctl is-active koris 2>/dev/null || echo "unknown")
