...
- Push to main, then deploy via: `git archive --format=tar HEAD | ssh root@91.107.168.34 "cd /opt/KorisPanel && tar xf -"` followed by `ssh root@91.107.168.34 "cd /opt/KorisPanel && go build -o /usr/local/bin/koris ./panel/cmd/panel && systemctl restart koris.service"`
