  ...
  <div class="page teleproxy-view">
    <header class="page-header">
      ...
      <div class="header-actions">
        <KButton v-if="hasProxies" variant="ghost" size="sm" :loading="rotatingAll" @click="rotateAll">{{ t('teleproxy.rotate_all') }}</KButton>
        <KButton variant="primary" size="sm" @click="showCreate = !showCreate">{{ t('teleproxy.add_proxy') }}</KButton>
      </div>
    </header>
    ...
    <section v-if="showCreate" class="card create-card">
      ...
      <div class="create-grid">
        <KSelect v-model="form.node_id" :options="nodeOptions" :placeholder="t('teleproxy.select_node')" />
        <KInput v-model.number="form.port" type="number" :placeholder="t('teleproxy.field_port')" />
        <KInput v-model="form.tag" :placeholder="t('teleproxy.tag_placeholder')" />
      </div>
      <div class="form-actions-row">
        <KButton variant="primary" size="sm" :loading="creating" @click="createProxy">{{ t('teleproxy.add_proxy') }}</KButton>
        <KButton variant="ghost" size="sm" @click="showCreate = false">Cancel</KButton>
      </div>
    </section>

    <div v-if="loading" class="skeleton-wrap">
      <KSkeleton v-for="i in 3" :key="i" variant="rect" width="100%" :height="56" />
    </div>
      ...
      <table class="k-table teleproxy-table">
        ...
        <tbody>
          <tr v-for="p in proxies" :key="p.id">
            ...
            <td>{{ p.port }}</td>
            <td>{{ p.tag || '—' }}</td>
            <td><KStatusPill :status="p.status" size="sm" /></td>
            <td>{{ p.connections_count }}</td>
            <td class="muted">{{ fmtDate(p.last_health_check) }}</td>
            <td>
              <div class="row-actions">
                <KButton v-if="!isActive(p)" variant="ghost" size="sm" @click="startProxy(p)">{{ t('teleproxy.start') }}</KButton>
                <KButton v-else variant="ghost" size="sm" @click="stopProxy(p)">{{ t('teleproxy.stop') }}</KButton>
                <KButton variant="ghost" size="sm" @click="copyLink(p)">{{ t('teleproxy.copy_link') }}</KButton>
                <KButton variant="danger" size="sm" @click="deleteProxy(p)">🗑</KButton>
              </div>
            </td>
          </tr>
        </tbody>
