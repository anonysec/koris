...
<template>
  <div :class="['page', 'customers-view', { 'customers-view--panel-open': detailPanelOpen }]">
      ...
      <Transition name="panel-slide">
        <div v-if="showAdvancedFilters" class="advanced-filters-panel" role="search" aria-label="Advanced filters">
          <div class="advanced-filters-grid">
            <FormField name="filter-plan" :label="t('customers.filter_plan')">
              <template #default="{ fieldId }">
                <Select :id="fieldId" v-model="filterPlanId" :options="planFilterOptions" />
              </template>
            </FormField>
            <FormField name="filter-date-from" :label="t('customers.filter_date_from')">
              <template #default="{ fieldId }">
                <input :id="fieldId" v-model="filterDateFrom" type="date" class="filter-date-input" />
              </template>
            </FormField>
            <FormField name="filter-date-to" :label="t('customers.filter_date_to')">
              <template #default="{ fieldId }">
      ...
      <Transition name="bulk-toolbar">
        <div v-if="hasSelection" class="bulk-toolbar" role="toolbar" aria-label="Bulk actions">
          ...
          <div class="bulk-toolbar__actions">
            <Button variant="ghost" size="sm" @click="executeBulkAction('enable')">{{ t('customers.enable') }}</Button>
            <Button variant="ghost" size="sm" @click="executeBulkAction('disable')">{{ t('customers.disable') }}</Button>
            <Button variant="ghost" size="sm" @click="executeBulkAction('traffic_reset')">{{ t('customers.traffic_reset') }}</Button>
            <Button variant="ghost" size="sm" @click="showBulkExtendSlide = true">{{ t('customers.bulk_extend') }}</Button>
            <Button variant="ghost" size="sm" @click="showBulkPlanSlide = true">{{ t('customers.bulk_change_plan') }}</Button>
            <Button variant="ghost" size="sm" @click="showBulkDataSlide = true">{{ t('customers.bulk_add_data') }}</Button>
            <Button variant="ghost" size="sm" @click="openBulkTagSlide">{{ t('customers.bulk_assign_tag') }}</Button>
            <Button variant="danger" size="sm" @click="executeBulkAction('delete')">{{ t('customers.delete') }}</Button>
          </div>
    ...
    <template v-if="currentMainTab === 'resellers'">
      ...
      >
        <template #cell-status="{ row }">
          <StatusPill :status="row.is_active ? 'active' : 'disabled'" size="sm" />
        </template>
        <template #cell-credit="{ value }">
        ...
        <template #cell-actions="{ row }">
          <div class="action-btns">
            <Button variant="ghost" size="sm" @click.stop="openEditReseller(row)">{{ t('btn.edit') }}</Button>
            <Button variant="ghost" size="sm" @click.stop="openCreditAdjust(row)">{{ t('resellers.credit') }}</Button>
            <Button variant="danger" size="sm" @click.stop="handleDeleteReseller(row.id, row.username)">{{ t('btn.delete') }}</Button>
          </div>
        </template>
      </DataTable>
    </template>
    ...
    <!-- New User Slide-Over -->
    <SlideOver :open="showUserSlideOver" :title="t('customers.new_user')" @close="showUserSlideOver = false">
      <form class="slide-form" autocomplete="off" @submit.prevent="handleCreateUser">
        ...
        <div class="slide-form__row">
          <FormField name="user-username" :label="t('user.username')" required>
            <template #default="{ fieldId }">
              <Input :id="fieldId" v-model="userForm.username" autocomplete="off" placeholder="username" />
            </template>
          </FormField>
          <FormField name="user-password" :label="t('user.password')" required>
            <template #default="{ fieldId }">
              <Input :id="fieldId" v-model="userForm.password" type="password" autocomplete="new-password" placeholder="••••••" />
            </template>
          </FormField>
        </div>
        ...
        <div class="slide-form__footer">
          <Button type="button" variant="ghost" @click="showUserSlideOver = false">{{ t('btn.cancel') }}</Button>
          <Button type="submit" variant="primary" :loading="saving">{{ t('btn.create') }}</Button>
        </div>
      </form>
    </SlideOver>
    ...
    >
      <form class="slide-form" autocomplete="off" @submit.prevent="handleResellerSubmit">
        <FormField name="reseller-username" :label="t('resellers.username')" required>
          <template #default="{ fieldId }">
            <Input :id="fieldId" v-model="resellerForm.username" autocomplete="off" placeholder="reseller_name" :disabled="!!editingResellerId" />
          </template>
        </FormField>
        <FormField name="reseller-password" :label="t('resellers.password')" :required="!editingResellerId">
          <template #default="{ fieldId }">
            <Input
              :id="fieldId"
        ...
        <FormField name="reseller-plan" :label="t('resellers.default_plan')" :hint="t('resellers.plan_hint')">
          <template #default="{ fieldId, describedBy }">
            <Select :id="fieldId" v-model="resellerForm.plan_id" :options="quotaPlanOptions" :placeholder="t('resellers.select_plan')" :aria-describedby="describedBy" />
          </template>
        </FormField>
        <FormField name="reseller-avatar" :label="t('resellers.avatar')" :hint="t('resellers.avatar_hint')">
          <template #default>
        ...
        <FormField v-if="editingResellerId" name="reseller-allowed-plans" :label="t('resellers.allowed_plans')" :hint="t('resellers.allowed_plans_hint')">
          <template #default>
        ...
        <div class="slide-form__footer">
          <Button type="button" variant="ghost" @click="showResellerSlideOver = false">{{ t('btn.cancel') }}</Button>
    ...
    <SlideOver :open="showCreditSlideOver" :title="`${t('resellers.adjust_credit')}: ${creditTarget?.username ?? ''}`" width="360px" @close="showCreditSlideOver = false">
      <form class="slide-form" @submit.prevent="handleCreditAdjust">
        <FormField name="credit-amount" :label="t('resellers.credit')" :hint="t('resellers.credit_hint')" required>
          <template #default="{ fieldId, describedBy }">
            <Input :id="fieldId" v-model="creditForm.amount" type="number" placeholder="10.00" :aria-describedby="describedBy" />
          </template>
        </FormField>
        <div class="slide-form__footer">
          <Button type="button" variant="ghost" @click="showCreditSlideOver = false">{{ t('btn.cancel') }}</Button>
          <Button type="submit" variant="primary" :loading="saving">{{ t('resellers.adjust_credit') }}</Button>
        </div>
      </form>
    </SlideOver>

    <!-- Bulk Extend Slide-Over -->
    <SlideOver :open="showBulkExtendSlide" :title="t('customers.bulk_extend')" width="360px" @close="showBulkExtendSlide = false">
      <form class="slide-form" @submit.prevent="executeBulkExtend">
        <p class="slide-form__hint">{{ selectedIds.length }} {{ t('customers.selected') }}</p>
        <FormField name="bulk-extend-days" :label="t('plans.duration_days')" required>
          <template #default="{ fieldId }">
            <Input :id="fieldId" v-model="bulkExtendDays" type="number" placeholder="30" />
          </template>
        </FormField>
        <div class="slide-form__footer">
          <Button type="button" variant="ghost" @click="showBulkExtendSlide = false">{{ t('btn.cancel') }}</Button>
          <Button type="submit" variant="primary" :loading="saving">{{ t('customers.bulk_extend') }}</Button>
        </div>
      </form>
    </SlideOver>

    <!-- Bulk Change Plan Slide-Over -->
    <SlideOver :open="showBulkPlanSlide" :title="t('customers.bulk_change_plan')" width="360px" @close="showBulkPlanSlide = false">
      <form class="slide-form" @submit.prevent="executeBulkChangePlan">
        <p class="slide-form__hint">{{ selectedIds.length }} {{ t('customers.selected') }}</p>
        <FormField name="bulk-plan" :label="t('user.plan')" required>
          <template #default="{ fieldId }">
            <Select :id="fieldId" v-model="bulkPlanId" :options="planOptions" :placeholder="t('resellers.select_plan')" />
          </template>
        </FormField>
        <div class="slide-form__footer">
          <Button type="button" variant="ghost" @click="showBulkPlanSlide = false">{{ t('btn.cancel') }}</Button>
          <Button type="submit" variant="primary" :loading="saving">{{ t('customers.bulk_change_plan') }}</Button>
        </div>
      </form>
    </SlideOver>

    <!-- Bulk Add Data Slide-Over -->
    <SlideOver :open="showBulkDataSlide" :title="t('customers.bulk_add_data')" width="360px" @close="showBulkDataSlide = false">
      <form class="slide-form" @submit.prevent="executeBulkAddData">
        <p class="slide-form__hint">{{ selectedIds.length }} {{ t('customers.selected') }}</p>
        <FormField name="bulk-data-gb" :label="t('plans.data_gb')" required>
          <template #default="{ fieldId }">
            <Input :id="fieldId" v-model="bulkAddDataGb" type="number" placeholder="10" />
          </template>
        </FormField>
        <div class="slide-form__footer">
          <Button type="button" variant="ghost" @click="showBulkDataSlide = false">{{ t('btn.cancel') }}</Button>
          <Button type="submit" variant="primary" :loading="saving">{{ t('customers.bulk_add_data') }}</Button>
        </div>
      </form>
    </SlideOver>

    <!-- Bulk Assign Tag Slide-Over -->
    <SlideOver :open="showBulkTagSlide" :title="t('customers.bulk_assign_tag')" width="400px" @close="showBulkTagSlide = false">
      <form class="slide-form" @submit.prevent="executeBulkAssignTag">
        <p class="slide-form__hint">{{ selectedIds.length }} {{ t('customers.selected') }}</p>
        <FormField name="bulk-tag" :label="t('tags.select_tag')" required>
          <template #default>
        ...
        <div class="slide-form__footer">
          <Button type="button" variant="ghost" @click="showBulkTagSlide = false">{{ t('btn.cancel') }}</Button>
          <Button type="submit" variant="primary" :loading="saving" :disabled="!bulkTagId">{{ t('customers.bulk_assign_tag') }}</Button>
        </div>
      </form>
    </SlideOver>
