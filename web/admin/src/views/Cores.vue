              ...
              >
                <div class="card-left">
                  ...
                  <span class="proto-meta">
                    ...
                    <StatusPill :status="entry.config[proto.key]?.enabled ? 'active' : 'inactive'" :label="entry.config[proto.key]?.enabled ? 'Enabled' : 'Disabled'" />
                  </span>
                </div>
                <button
                  class="toggle-btn"
    ...
    >
          ...
          >
            <FormField :label="t(field.label)" :name="field.key">
              ...
              <div v-else-if="field.type === 'password' || (field.key === 'psk')" class="password-field">
                ...
                />
                <Button size="sm" variant="ghost" @click="generatePsk">
      ...
      <template #footer>
        <div class="panel-footer">
          ...
          <Button :loading="panelSaving" @click="saveProtocolSettings">
            {{ t('nodes.save_config') }}
          </Button>
        </div>
      </template>
    ...
    >
      <div class="panel-body">
        <form autocomplete="off" @submit.prevent>
          <FormField :label="t('nodes.node_name')" name="edit-name">
            <Input v-model="editNodeForm.name" type="text" autocomplete="off" />
          </FormField>
          <FormField :label="t('nodes.address')" name="edit-address">
            <Input v-model="editNodeForm.address" type="text" autocomplete="off" />
          </FormField>
          <FormField label="Port" name="edit-port">
            <Input v-model="editNodeForm.port" type="number" autocomplete="off" />
          </FormField>
          <FormField label="API Key" name="edit-apikey">
            <Input v-model="editNodeForm.api_key" type="text" placeholder="Leave empty to keep current" autocomplete="off" />
          </FormField>
          <FormField label="Certificate" name="edit-ca">
      ...
      <template #footer>
        <div class="panel-footer">
          <Button variant="ghost" @click="editNodeOpen = false">Cancel</Button>
          <Button :loading="editNodeSaving" @click="saveEditNode">
            {{ t('nodes.save_config') }}
          </Button>
        </div>
      </template>
