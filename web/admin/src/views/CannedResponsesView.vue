...
<template>
  <div class="page canned-view">
    ...
    <div v-if="loading" class="skeleton-wrap">
      <KSkeleton variant="table-row" :count="4" />
    </div>
    ...
    <div v-else class="responses-list">
      ...
      >
        ...
        <div class="response-card__actions">
          <KButton variant="ghost" size="sm" @click="openEdit(response)">
            ✏️ {{ t('canned.edit') }}
          </KButton>
          <KButton variant="ghost" size="sm" @click="deleteResponse(response)">
            🗑️ {{ t('canned.delete') }}
          </KButton>
        </div>
      </div>
    ...
    >
      <form class="canned-form" @submit.prevent="saveResponse">
        <KFormField name="title" :label="t('canned.field_title')">
          <template #default="{ fieldId }">
            <KInput :id="fieldId" v-model="formTitle" :placeholder="t('canned.title_placeholder')" />
          </template>
        </KFormField>

        <KFormField name="category" :label="t('canned.field_category')">
          <template #default="{ fieldId }">
            <KSelect :id="fieldId" v-model="formCategory" :options="categoryOptions" />
          </template>
        </KFormField>

        <KFormField name="body" :label="t('canned.field_body')">
          <template #default="{ fieldId }">
            <KTextarea
              :id="fieldId"
              v-model="formBody"
        ...
        <div v-if="formBody.includes('{{') " class="preview-section">
          <KFormField name="preview-vars" :label="t('canned.preview_vars')">
            <template #default="{ fieldId }">
              <KTextarea
                :id="fieldId"
                v-model="previewVars"
