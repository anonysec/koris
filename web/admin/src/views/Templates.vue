  ...
  <div class="page templates-view">
    ...
    <div v-if="showForm" class="template-form-panel">
      ...
      <form class="template-form" @submit.prevent="handleSubmit">
        <div class="form-grid">
          <FormField name="tpl-name" :label="t('templates.name')" required>
            <template #default="{ fieldId }">
              <Input :id="fieldId" v-model="form.name" :placeholder="t('templates.name_placeholder')" />
            </template>
          </FormField>
          <FormField name="tpl-plan" :label="t('templates.plan')">
            <template #default="{ fieldId }">
              <Select
                :id="fieldId"
          ...
          <FormField name="tpl-status" :label="t('templates.status')" required>
            <template #default="{ fieldId }">
              <Select
                :id="fieldId"
                v-model="form.status"
          ...
          <FormField name="tpl-conn-limit" :label="t('templates.conn_limit')">
            <template #default="{ fieldId }">
              <Input :id="fieldId" v-model="form.connection_limit" type="number" :placeholder="t('templates.unlimited_placeholder')" />
            </template>
          </FormField>
        </div>
        <FormField name="tpl-radius-checks" :label="t('templates.radius_checks')">
          <template #default="{ fieldId }">
            <Textarea
              :id="fieldId"
        ...
        <FormField name="tpl-radius-replies" :label="t('templates.radius_replies')">
          <template #default="{ fieldId }">
            <Textarea
              :id="fieldId"
    ...
    <div v-if="showDeleteConfirm" class="confirm-overlay" @click.self="cancelDelete">
      <div class="confirm-dialog">
        ...
        <div class="confirm-actions">
          <Button variant="ghost" @click="cancelDelete">{{ t('btn.cancel') }}</Button>
          <Button variant="danger" @click="executeDelete">{{ t('btn.delete') }}</Button>
        </div>
      </div>
    </div>
    ...
    >
      ...
      <template #cell-actions="{ row }">
        <div class="action-btns">
          <Button variant="ghost" size="sm" @click.stop="openEdit(row)">{{ t('btn.edit') }}</Button>
          <Button variant="danger" size="sm" @click.stop="confirmDelete(row)">{{ t('btn.delete') }}</Button>
        </div>
      </template>
    </DataTable>
  </div>
