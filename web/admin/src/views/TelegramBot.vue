    ...
    <div class="tg-grid">
      <section class="card tg-config">
        ...
        <form class="settings-form" autocomplete="off" @submit.prevent="saveSettings">
          <FormField name="tg-token" label="Bot Token" hint="Get the token from @BotFather">
            <template #default="{ fieldId }">
              <Input :id="fieldId" v-model="telegramToken" placeholder="123456:ABC-DEF..." type="password" autocomplete="new-password" />
            </template>
          </FormField>
          <FormField name="tg-chat" label="Admin Chat / Group ID" hint="Chat or group ID that receives alerts">
            <template #default="{ fieldId }">
          ...
          <div class="form-actions-row">
            ...
            <Button type="button" variant="ghost" size="sm" :loading="testing" @click="testBot">Test &amp; Restart</Button>
          </div>
        </form>
      </section>
