  ...
  <div class="page node-groups-view">
    ...
    <section class="assign-section">
      ...
      <div class="assign-form">
        <FormField name="assign-node" :label="t('node_groups.node')">
          <template #default="{ fieldId }">
            <Select :id="fieldId" v-model="assigningNodeId" :options="nodeOptions" :placeholder="t('node_groups.select_node')" />
          </template>
        </FormField>
        <FormField name="assign-group" :label="t('node_groups.group')">
          <template #default="{ fieldId }">
            <Select :id="fieldId" v-model="assigningGroupId" :options="groupOptions" :placeholder="t('node_groups.select_group')" />
          </template>
        </FormField>
        <Button
          variant="primary"
    ...
    <section class="groups-section">
      <div v-if="loading && groups.length === 0">
        <Skeleton v-for="i in 3" :key="i" variant="rect" width="100%" :height="72" />
      </div>
      ...
      <div v-else class="groups-grid">
        <div v-for="group in groups" :key="group.id" class="group-card">
          ...
          <div class="group-card__actions">
            <Button variant="ghost" size="sm" @click="openEditModal(group)">{{ t('btn.edit') }}</Button>
            <Button variant="danger" size="sm" @click="deleteGroup(group)">{{ t('btn.delete') }}</Button>
          </div>
        </div>
      </div>
    </section>
    ...
    <section class="load-section">
      ...
      <div v-if="loadingStats">
        <Skeleton v-for="i in 2" :key="i" variant="rect" width="100%" :height="48" />
      </div>
        ...
        >
          <div class="modal">
            ...
            <form class="modal__body" @submit.prevent="saveGroup">
              <FormField name="group-name" :label="t('node_groups.name')" required>
                <template #default="{ fieldId }">
                  <Input :id="fieldId" v-model="groupForm.name" :placeholder="t('node_groups.name_placeholder')" />
                </template>
              </FormField>
              <FormField name="group-region" :label="t('node_groups.region')">
                <template #default="{ fieldId }">
                  <Input :id="fieldId" v-model="groupForm.region" :placeholder="t('node_groups.region_placeholder')" />
                </template>
              </FormField>
              <FormField name="group-desc" :label="t('node_groups.description')">
                <template #default="{ fieldId }">
                  <Textarea :id="fieldId" v-model="groupForm.description" :rows="3" />
                </template>
              </FormField>
              <FormField name="group-lb" :label="t('node_groups.load_balancing')">
                <template #default>
              ...
              <div class="modal__actions">
                <Button variant="ghost" @click="closeModal">{{ t('btn.cancel') }}</Button>
                <Button type="submit" variant="primary" :loading="saving">{{ t('btn.save') }}</Button>
              </div>
            </form>
          </div>
        </div>
