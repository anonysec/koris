  ...
  <div class="page plans-view">
    ...
    <div v-if="showForm" class="plan-form-panel">
      ...
      <form class="plan-form" @submit.prevent="handleSubmit">
        ...
        <div class="form-grid">
          <FormField name="plan-name" :label="t('plans.name')" required>
          ...
          <template v-if="!isPayg">
            <FormField name="plan-data" :label="t('plans.data_gb')" required>
              <template #default="{ fieldId }">
                <Input :id="fieldId" v-model="form.data_gb" type="number" placeholder="50" />
              </template>
            </FormField>
            <FormField name="plan-duration" :label="t('plans.duration_days')" required>
              <template #default="{ fieldId }">
                <Input :id="fieldId" v-model="form.duration_days" type="number" placeholder="30" />
              </template>
            </FormField>
            <FormField name="plan-price" :label="t('plans.price')" required>
              <template #default="{ fieldId }">
                <Input :id="fieldId" v-model="form.price" type="number" placeholder="9.99" />
              </template>
            </FormField>
          </template>
          ...
          <template v-if="isPayg">
            <FormField name="plan-price-per-gb" :label="t('plans.price_per_gb')" required>
              <template #default="{ fieldId }">
                <Input :id="fieldId" v-model="form.price_per_gb" type="number" step="0.01" placeholder="0.50" />
              </template>
            </FormField>
            <FormField name="plan-price-per-day" :label="t('plans.price_per_day')" required>
              <template #default="{ fieldId }">
                <Input :id="fieldId" v-model="form.price_per_day" type="number" step="0.01" placeholder="0.10" />
              </template>
            </FormField>
            <FormField name="plan-disconnect" :label="t('plans.disconnect_on_zero')">
              <template #default>
        ...
        <div class="form-actions">
          <Button variant="ghost" @click="resetForm">{{ t('btn.cancel') }}</Button>
    ...
    <div v-if="store.loading && store.list.length === 0" class="plans-grid">
      <Skeleton v-for="i in 4" :key="i" variant="rect" :width="'100%'" :height="160" />
    </div>
    ...
    <div v-else ref="plansContainerRef" class="plans-grid" :class="{ 'plans-grid--dragging': plansDragging }">
      <div v-for="plan in store.list" :key="plan.id" class="plan-card" :class="{ 'plan-card--inactive': !plan.is_active }">
        ...
        <div class="plan-card__actions">
          <Button variant="ghost" size="sm" @click="openEdit(plan)">{{ t('btn.edit') }}</Button>
          <Button v-if="plan.is_active" variant="danger" size="sm" @click="deactivatePlan(plan.id)">{{ t('plans.deactivate') }}</Button>
        </div>
      </div>
    </div>
