...
<template>
  <div class="sp">
    ...
    <section class="sp__section">
      ...
      <div class="sp__account-grid">
        ...
        <div class="sp__account-item">
          ...
          <StatusPill :status="statusVariant">{{ t(`portal.account.status_${status}`) }}</StatusPill>
        </div>
        <div class="sp__account-item">
        ...
        <div class="sp__account-item">
          ...
          <StatusPill :status="isOnline ? 'active' : 'disabled'">
    ...
    <section class="sp__section">
      ...
      <Skeleton v-if="usageStore.loading && !usageStore.usage" type="card" :count="1" />
      <template v-else>
        <div class="sp__usage-compact">
          ...
          <UsageBar :used="totalUsageBytes" :limit="maxDataBytes" :show-label="false" />
    ...
    <section class="sp__section">
      ...
      <Skeleton v-if="profilesLoading && !profiles.length" type="card" :count="2" />
      <template v-else>
        <!-- Auth mode label -->
        ...
        <div v-else class="sp__profiles-list">
          ...
          <div v-for="peer in peers" :key="'wg-' + peer.id" class="sp__profile-card">
            ...
            <Button variant="primary" size="sm" @click="downloadConfig(peer.id)">
    ...
    <section v-if="isFull && (telegramProxies.length || teleProxiesLoading)" class="sp__section">
      ...
      <Skeleton v-if="teleProxiesLoading && !telegramProxies.length" type="card" :count="1" />
        ...
        <div v-for="proxy in telegramProxies" :key="proxy.id" class="sp__teleproxy-card">
          ...
          <div class="sp__teleproxy-info">
            ...
            <div class="sp__teleproxy-meta">
              <StatusPill :status="proxy.status === 'active' ? 'active' : 'disabled'" size="sm">
                {{ t(`portal.teleproxy.status_${proxy.status === 'active' ? 'active' : 'stopped'}`) }}
              </StatusPill>
            </div>
          </div>
          <Button variant="primary" size="sm" @click="copyProxyLink(proxy)">
    ...
    <section v-if="isFull" class="sp__section">
      ...
      <Skeleton v-if="ticketsStore.loading && !ticketsStore.list.length" type="card" :count="1" />
      ...
      <template v-else>
        ...
        <div v-if="ticketsStore.detail" class="sp__ticket-detail">
          ...
          <div class="sp__ticket-header">
            ...
            <StatusPill :status="ticketsStore.detail.status === 'open' || ticketsStore.detail.status === 'in_progress' ? 'active' : 'disabled'" size="sm">
          ...
          >
            ...
            <div class="sp__reply-actions">
              <Button type="submit" variant="primary" size="sm" :loading="ticketsStore.loading" :disabled="!replyMessage.trim()">
          ...
          >
            ...
            <Button variant="primary" size="sm" :disabled="ratingValue < 1" :loading="ticketsStore.loading" @click="handleRate">
          ...
          <div v-if="ticketsStore.list.length" class="sp__tickets-list">
            <div v-for="ticket in ticketsStore.list" :key="ticket.id" class="sp__ticket-row" @click="handleViewTicket(ticket.id)">
              <div class="sp__ticket-row-info">
                ...
                <StatusPill :status="ticket.status === 'open' || ticket.status === 'in_progress' ? 'active' : ticket.status === 'waiting' ? 'expired' : 'disabled'" size="sm">
        ...
        <form v-if="showCreateForm && !ticketsStore.detail" class="sp__ticket-form" @submit.prevent="handleCreateTicket">
          ...
          <FormField :label="t('portal.support.message')" :required="true">
            <Textarea v-model="ticketForm.body" :placeholder="t('portal.support.messagePlaceholder')" :rows="4" />
          </FormField>
          <div class="sp__form-actions">
            <Button variant="ghost" size="sm" @click="showCreateForm = false">{{ t('portal.support.cancel') }}</Button>
            <Button type="submit" variant="primary" size="sm" :loading="ticketsStore.loading" :disabled="!ticketForm.subject || !ticketForm.body">
