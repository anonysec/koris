...
<template>
  <div class="billing">
    ...
    <Skeleton v-if="billing.loading && !billing.payments.length" type="card" :count="2" />
    ...
    <template v-else>
      ...
      <section class="billing__section">
        ...
        <form class="billing__form" @submit.prevent="handleRenew">
          ...
          <Button type="submit" variant="primary" :loading="billing.loading" :disabled="!renewPlanId">
            Activate Plan
          </Button>
        </form>
      </section>
      ...
      <section class="billing__section">
        ...
        <form class="billing__form" @submit.prevent="handleSubmitPayment">
          <FormField label="Amount" :required="true">
            <Input v-model.number="paymentForm.amount" type="number" :min="1" placeholder="Amount" />
          </FormField>
          ...
          <FormField label="Receipt / Reference">
            <Input v-model="paymentForm.receipt" placeholder="Transfer reference or receipt ID" />
          </FormField>

          <Button type="submit" variant="primary" :loading="billing.loading" :disabled="paymentForm.amount <= 0">
            Submit Payment
          </Button>
        </form>
      </section>
      ...
      <section class="billing__section">
        ...
        >
          ...
          <template #cell-status="{ row }">
            <StatusPill :status="row.status === 'approved' ? 'active' : row.status === 'rejected' ? 'disabled' : 'expired'">
              {{ row.status }}
            </StatusPill>
          </template>
    ...
    >
      ...
      <template #footer>
        <div class="billing__modal-actions">
          <Button variant="ghost" @click="showRenewalModal = false">Cancel</Button>
          <Button
            variant="primary"
            :loading="confirming"
            :disabled="confirming"
    ...
    >
      ...
      <template #footer>
        <div class="billing__modal-actions">
          <Button variant="ghost" @click="showTopupModal = false">Cancel</Button>
          <Button
            variant="primary"
            :loading="confirming"
            :disabled="confirming"
