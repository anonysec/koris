...
<template>
  <div class="invoices">
    ...
    <Skeleton v-if="loading && !invoices.length" type="table" :count="5" />
      ...
      <div v-if="showDetail && selectedInvoice" class="invoices__detail-overlay" @click.self="closeDetail">
        <div class="invoices__detail-card">
          <div class="invoices__detail-header">
            ...
            <Button variant="ghost" size="sm" @click="closeDetail">✕</Button>
          </div>

          <div class="invoices__detail-body">
            <div class="invoices__detail-row">
              <span class="invoices__detail-label">Status</span>
              <StatusPill :status="getStatusVariant(selectedInvoice.status)">
      ...
      <section class="invoices__section">
        ...
        >
          ...
          <template #cell-status="{ row }">
            <StatusPill :status="getStatusVariant(row.status)">
              {{ row.status }}
            </StatusPill>
          </template>
          ...
          <template #cell-actions="{ row }">
            <Button variant="ghost" size="sm" @click="viewInvoice(row)">
              View
            </Button>
          </template>
        </DataTable>
