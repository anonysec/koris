...
func buildTLSConfig(cfg NodeConfig) (*tls.Config, error) {
	...
	if len(cfg.CACert) > 0 {
		...
		// No CA cert provided. Refuse to connect rather than skipping
		// server verification: an unauthenticated knode TLS endpoint is
		// a MITM risk. The admin must provide the node's CA (or a
		// self-signed API cert generated by the node).
		return nil, errors.New("no CA certificate configured for node; refusing to connect without server verification")
	}
